...
module.exports = class ServiceGenerator extends Generator {
    ...
    if (mainExpression.length !== 1) {
      this.log
        ...
        .conflict(
          `${
            this.libDirectory
          }/services/index.js seems to have more than one function declaration and we can not register the new service. Did you modify it?`
        )
  ...
  writing() {
    ...
    const mainFile = this.destinationPath(
      ...serviceFolder,
      `${kebabName}.service.js`
    );
    ...
    if (!this.fs.exists(mainFile)) {
      const servicejs = this.destinationPath(
        this.libDirectory,
        "services",
        "index.js"
      );
    ...
    if (fs.existsSync(path.join(templatePath, "types", `${adapter}.js`))) {
      ...
      this.fs.copyTpl(this.templatePath("service.js"), mainFile, context);
    }
